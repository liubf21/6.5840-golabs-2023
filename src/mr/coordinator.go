...
func (c *Coordinator) GetTask(args *GetTaskRequest, reply *GetTaskResponse) error {
	...
	if len(c.remainingTasks) > 0 { // assign a task
		...
		// log.Printf("%v finished, schedule the next phase", c.phase)
		if c.phase == MapPhase {
			c.phase = ReducePhase
			c.Schedule()
